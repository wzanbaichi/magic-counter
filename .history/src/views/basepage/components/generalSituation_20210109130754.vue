      ...
      myChart.setOption(
        {
          ...
          tooltip: {
            ...
            formatter: (params)=>{
              console.log(params)
              const tooltops = `<span style="margin-right:5px;display:inline-block;width:10px;height:10px;border-radius:5px;background-color:rgba(220, 220, 220, 0.8);"></span>${params[0].value}全 ${(params[0].value/sum*100).toFixed(2)}%`
              return t
            }
          },
          xAxis: {
