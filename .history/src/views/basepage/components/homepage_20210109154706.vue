    ...
    >
      ...
      <div class="count_info_container">
        <div class="price">
          ...
          <at-input
            v-model.number="counterParams.price"
            size="small"
            placeholder="请输入全年单价"
            class="price_input"
          >
        ...
        <div class="coupon">
          ...
          <div class="coupon_input_list">
            ...
            >
              ...
              <at-input
                v-model.number="counterParams.couponData[month]"
                size="small"
                placeholder="请输入全年单价"
                class="price_input"
              >
...
export default {
  ...
  computed: {
    ...
    fileInfo() {
      return this.$store.state.app.fileInfo
    }
  },
  watch: {
    bookData: {
      handler(newData) {
        ...
        this.$store.commit('app/SET_MONTH_LIST', this.monthList)
        const sheetsListData = {}
        const couponData = {}
        this.monthList.forEach(
          item => {
            sheetsListData[item] = newData.Sheets[item]
            couponData[item] = this.counterParams.couponData[item] || 1200
          }
        )
        this.sheetsListData = sheetsListData
        this.counterParams.couponData = couponData
  ...
  mounted() {
    if (this.fileFigureOut) {
      this.monthList = this.$store.state.app.monthList
      this.counterParams = this.$store.state.app.counterParams
    }
  },
  methods: {
    handleUploadFiles() {
      ...
      input.addEventListener('change',
        (e) => {
          ...
          this.$store.commit('app/SET_FILE_INFO', file)
        ...
        month => {
          ...
          for (let i = 0; i < this.monthDataNumberData[month]; i++) {
            ...
            const rowData = {
              ...
              qkrq: monthData[`V${i + 3}`] && monthData[`V${i + 3}`] !== '/' ? new Date(monthData[`V${i + 3}`].w) : monthData[`V${i + 3}`],
              demoRq: monthData[`X${i + 3}`] && monthData[`X${i + 3}`] !== '/' ? new Date(monthData[`X${i + 3}`].w) : monthData[`V${i + 3}`],
              _demoRq: monthData[`X${i + 3}`] ,
              _demoRq: monthData[`X${i + 3}`] && monthData[`X${i + 3}`] !== '/' ? new Date(monthData[`X${i + 3}`].w) : monthData[`V${i + 3}`],
