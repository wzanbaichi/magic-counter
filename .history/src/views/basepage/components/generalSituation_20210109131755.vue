      ...
      myChart.setOption(
        {
          ...
          tooltip: {
            ...
            formatter: (params) => {
              console.log(params)
              const tooltips = `<span style="margin-right:5px;display:inline-block;width:10px;height:10px;border-radius:5px;background-color:#748EDE;"></span>${params[0].name}${params[0].value}  ${(params[0].value / sum * 100).toFixed(2)}%`
              return tooltips
            }
          },
          xAxis: {
