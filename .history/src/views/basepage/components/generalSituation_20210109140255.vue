<template>
  <div>
    <at-card :bodyStyle="{padding:'12px'}">
      ...
      <h4 v-if="qkAndBqkSum" slot="extra">全年共计:{{ qkAndBqkSum }}</h4>
      <div
        id="chart"
        class="chart"
      >
...
export default {
  ...
  methods: {
    ...
    countQkAndBqkData() {
      ...
      this.qkAndBqkSum = yData.reduce((prev, next, index, ary) => prev + next)
      ...
      myChart.setOption(
        {
          ...
          tooltip: {
            ...
            formatter: (params) => {
              ...
              数量：${params[0].value}
              <br/>
              占比：${(params[0].value / qkAndBqkSum * 100).toFixed(2)}%`
              return tooltips
            }
          },
          xAxis: {
