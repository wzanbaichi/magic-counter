...
export default {
  ...
  methods: {
    ...
    counterDataAndInitCharts() {
          ...
          this.monthDetailData[month].forEach(
            row => {
              ...
              if (row.qk >= currentMinPrice || row.bqk >= currentMinPrice) {
                ...
                if(currentMonthStudentName.indexOf(row))
              }
      ...
      this.qkAndBqkSum = qkAndBqkData.reduce((prev, next, index, ary) => add(prev, next))
      this.tfjeSum = tfjeData.reduce((prev, next, index, ary) => add(prev, next))
      this.revenueSum = revenueData.reduce((prev, next, index, ary) => add(prev, next))
      this.zcjeSum = zcjeData.reduce((prev, next, index, ary) => add(prev, next))
      this.zrjeSum = zrjeData.reduce((prev, next, index, ary) => add(prev, next))
      /**
       * 全款及补全款图表
       */
